...
conditions       <- as.vector (read.table(input, header=F, sep = "\t", row.names = 1, check.name = F, comment.char ="", nrow = 1))
...
baseMean = cbind(rowMeans(data.frame(counts(dds,normalized=TRUE)[,dds$condition == case])), rowMeans(data.frame(counts(dds,normalized=TRUE)[,dds$condition == control])))
colnames(baseMean) <-  c("baseMeanA", "baseMeanB")

degs               <- paste(result, ".txt", sep="")
write.table(cbind(ID = rownames(degs_val), normalized.counts, baseMean, degs_val[, c("baseMean", "log2FoldChange", "pvalue", "padj")]), 
